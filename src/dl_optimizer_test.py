...
def test_adam_optimizer():
    ...
    npt.assert_equal(optimizer.weightsMomentum[layerIndex], expected_weights_momentum)

    expected_biases_momentum = momentum * optimizerCopy.biasesMomentum[layerIndex] + (1-momentum) * db
    npt.assert_equal(optimizer.biasesMomentum[layerIndex], expected_biases_momentum)

    expected_weights_scale = scale * optimizerCopy.weightsScale[layerIndex] + (1-scale) * (dw**2)
    npt.assert_equal(optimizer.weightsScale[layerIndex], expected_weights_scale)

    expected_biases_scale = scale * optimizerCopy.biasesScale[layerIndex] + (1-scale) * (db**2)
    npt.assert_equal(optimizer.biasesScale[layerIndex], expected_biases_scale)
    ...
    npt.assert_equal(expected_weights_momentum_corrected, weightsMomentumCorrected)
    ...
    npt.assert_equal(expected_biases_momentum_corrected, biasesMomentumCorrected)    
    ...
    npt.assert_equal(expected_weights_scale_corrected, weightsScaleCorrected)
    ...
    npt.assert_equal(expected_biases_scale_corrected, biasesScaleCorrected)
